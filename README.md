...
*User A* should download and unzip the project files. 
...
If needed, *User A* should move the files/directory to a desired location on *User A*'s computer either using the command line or a file browser.

*User A* should create an R project for the `collaborating_using_renv` folder.

Once the files are in the desired location and an R project has been created, confirm that *User A* can build the report by executing `make`.
...
2. In an R console, use `getwd` to confirm that the current working directory in the R console is the project directory. If you created an R project, this should already have happened by default.
...
4. *User B* should confirm that a folder called `<user_b_repo>` was added to the current working directory of their terminal.
  - E.g., use `cd <user_b_repo>` and `ls` to change working directory into the newly downloaded repository and list its contents.

5. Be sure to use the `git@github.com:<user_b_name>/<user_b_repo>` syntax and __not__ `https://github.com/<user_b_name>/<user_b_repo>` syntax. You can confirm what web address was used to add the remote by executing `cd <user_b_repo>` and `git remote -v`.
...
2. Confirm that *User B* has the `renv` package installed.
	- e.g., you could run `"renv" %in% row.names(installed.packages())`
	- If the above command returns `FALSE`, install the `renv` package using `install.packages('renv')`
...
4. Once you are __positive__ that you are in the correct working directory, restore the package library using `renv::restore()`.
	...
	- use google/chat gpt/David/TAs as appropriate to help with package install errors

5. Once the environment is synchronized, confirm that the report builds, e.g., by running `make` or by executing the contents of `code/01_make_output.R` and `code/02_render_report.R`.
...
6. Run `renv::status()` to check whether your project is synchronized with the lock file. 
	- You should get a message saying that there are packages used that are not recorded in the lock file.
7. Run `renv::snapshot()` to update the lock file.

Now *User B* is ready to submit a PR to *User A*.

8. Use `git add` and `git commit` to make a new commit along the `wes` branch that includes updates to `report.Rmd` and `renv.lock`.
	- include a meaningful commit message
9. Push the `wes` branch to GitHub.
	- `git push origin wes`
10. Submit a pull request to *User A*'s repository.
...
11. Make sure the working directory of your terminal is your project directoy. 
12. Use `mv report.html report_wes.html` to rename the report.
13. Submit `report_wes.html` as part of the assignment.
...
1. *User A* should add *User B*'s repository as a remote.
	- `git remote add <user_b_remote_name> git@github.com:<user_b_name>/<user_b_repo>`
